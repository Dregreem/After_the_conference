...
function [P_watts, props] = pvTrackerPower(G, theta_inc_deg, panel_config)
% PVTRACKERPOWER - Computes instantaneous power for the TRACKING panel
%
% Input:
%   G             - irradiance W/m² (from PVGIS)
%   theta_inc_deg - incidence angle in degrees (from checkAlignment output)
%   panel_config  - struct with fields: A (m²), eta (0-1)
%
% Output:
%   P_watts - instantaneous power in Watts

    cos_theta = max(0, cosd(theta_inc_deg));
    P_watts = panel_config.A * panel_config.eta * G * cos_theta;
