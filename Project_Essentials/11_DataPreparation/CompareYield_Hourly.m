%% ╔══════════════════════════════════════════════════════════════════════╗
%% ║  CompareYield_Hourly.m                                               ║
%% ║  FIXED vs TRACKER — WITH CELL TEMPERATURE MODEL (HOURLY-AVERAGED)    ║
%% ║                                                                       ║
%% ║  Base: CompareYield_TMY_Temp.m                                        ║
%% ║  Data: prepareDataForAnalysis_Hourly() → prepared_data_HOURLY_*.mat  ║
%% ║                                                                       ║
%% ║  Key Features:                                                        ║
%% ║    • Loads pre-processed HOURLY data (hourly-averaged temperatures)  ║
...
%% ║                                                                       ║
%% ║  Data Source: HOURLY-AVERAGED (1-second PVGIS + hourly T/WS)         ║
%% ║                                                                       ║
%% ╚══════════════════════════════════════════════════════════════════════╝
...
if ~exist('loc_name', 'var'), loc_name = 'YTU_FROM_NASA'; end
if exist('RESULTS_BASE','var')
    res_dir = fullfile(RESULTS_BASE, 'Results', upper(loc_name), 'Hourly');
else
    res_dir = fullfile(fileparts(mfilename('fullpath')), 'Results', upper(loc_name), 'Hourly');
end
...
fprintf('\n╔══════════════════════════════════════════════════════════════╗\n');
fprintf('║    FIXED vs TRACKER — HOURLY DATA (with Temperature)         ║\n');
...
% Önce HOURLY veri dosyasını hazırla (gerekirse)
file_hourly = sprintf('prepared_data_HOURLY_%s.mat', upper(loc_name));

% Akıllı arama: var olan dosyaları kontrol et (NASA ve PVGIS klasörleri)
file_options = {
    fullfile('DataPreparation', 'NASA', 'prepared_data_HOURLY_NASA_YTU.mat'),     % NASA format
    fullfile('DataPreparation', 'NASA', file_hourly),                             % NASA standard format
    fullfile('DataPreparation', 'PVGIS', file_hourly),                            % PVGIS standard format
    fullfile('DataPreparation', 'prepared_data_HOURLY_NASA_YTU.mat'),             % Old location
    fullfile('DataPreparation', file_hourly),                                     % Standard format
    file_hourly,                                                                   % Current dir standard
    'prepared_data_HOURLY_NASA_YTU.mat',                                          % NASA format current
    fullfile('..', file_hourly),                                       % Parent dir
    fullfile('..', 'prepared_data_HOURLY_NASA_YTU.mat')               % Parent NASA format
};

file_hourly_found = '';
for i = 1:length(file_options)
    if isfile(file_options{i})
        file_hourly_found = file_options{i};
        fprintf('  ✓ Mevcut veri bulundu: %s\n', file_hourly_found);
        break;
    end
end

% Dosya bulunamadı ise oluştur
if isempty(file_hourly_found)
    fprintf('  ℹ Hazırlanmış veri bulunamadı, oluşturuluyor...\n');
    PreparedData = prepareDataForAnalysis_Hourly(loc_name);
    file_hourly_found = file_hourly;
    save(file_hourly_found, 'PreparedData');
    fprintf('  ✓ Veri hazırlandı ve kaydedildi: %s\n\n', file_hourly_found);
end

% Şimdi yükle
try
    load(file_hourly_found, 'PreparedData');
catch ME
    error('Failed to load prepared data: %s\n%s', file_hourly_found, ME.message);
end
...
for d = 1:nDays
    ...
    csv_detail_file = fullfile(res_dir, ...
        sprintf('DetailedTimestep_%s_Month%02d_Hourly.csv', loc_name, d));
    T_detail = table( ...
        time_vec, ...
...
fprintf('════════════════════════════════════════════════════════════════\n');
fprintf('  ANNUAL YIELD SUMMARY (HOURLY DATA WITH TEMPERATURE) — %s\n', Geo.Name);
fprintf('════════════════════════════════════════════════════════════════\n');
...
%% ── SAVE RESULTS .mat ────────────────────────────────────────────────────
mat_file = fullfile(res_dir, sprintf('%s_Results_Hourly.mat', upper(loc_name)));
...
title(axT1, sprintf('Monthly Average Cell Temperature — %s, TR (Hourly Data)', Geo.Name), ...
    'FontName',FN,'FontSize',FSt,'FontWeight','bold');
legend(axT1,'Location','northwest','FontName',FN,'FontSize',FSr,'Box','on');
exportgraphics(figT1, fullfile(res_dir,'FigT1_CellTemperature_Hourly.png'), 'Resolution',300);
close(figT1);
fprintf('  ✓ FigT1_CellTemperature_Hourly.png\n');
...
title(axT2, sprintf('Monthly Temperature-Corrected Panel Efficiency — %s, TR (Hourly)', Geo.Name), ...
    'FontName',FN,'FontSize',FSt,'FontWeight','bold');
legend(axT2,'Location','best','FontName',FN,'FontSize',FSr,'Box','on');
exportgraphics(figT2, fullfile(res_dir,'FigT2_PanelEfficiency_Hourly.png'), 'Resolution',300);
close(figT2);
fprintf('  ✓ FigT2_PanelEfficiency_Hourly.png\n');
...
title(axT3, sprintf('Monthly Net Gain with Cell Temperature Correction — %s, TR (Hourly)', Geo.Name), ...
    'FontName',FN,'FontSize',FSt,'FontWeight','bold');
exportgraphics(figT3, fullfile(res_dir,'FigT3_NetGain_Hourly.png'), 'Resolution',300);
close(figT3);
fprintf('  ✓ FigT3_NetGain_Hourly.png\n');
