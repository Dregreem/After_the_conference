%% ╔══════════════════════════════════════════════════════════════════════╗
%% ║  CompareYield_TMY_Temp.m                                             ║
%% ║  Branch of CompareYield_TMY.m — adds cell-temperature model          ║
%% ║                                                                       ║
%% ║  ADDITIONS vs original:                                               ║
%% ║    • Loads YTU_MonthlyAvg.csv alongside the _Data.csv to get         ║
%% ║      T2m and WS10m (monthly-hourly averages)                          ║
...
%% ║    • Applied to BOTH fixed and tracker power calculations              ║
%% ║    • Extra logged vectors: T_amb, T_cell_fix, T_cell_trk, eta_eff    ║
%% ║    • Extra Results fields: T_cell_mean, eta_mean, E_fixed_temp,       ║
%% ║      E_net_temp, Gain_temp                                            ║
%% ║    • DetailedTimestep CSV now includes temperature columns             ║
%% ║    • Extra figure: Fig_Temp — monthly cell temp & efficiency impact    ║
%% ║                                                                       ║
%% ║  ORIGINAL FILE IS UNTOUCHED — run CompareYield_TMY.m for the          ║
%% ║  pure irradiance/kinematics version.                                   ║
%% ╚══════════════════════════════════════════════════════════════════════╝
...
fprintf('╔══════════════════════════════════════════════════════════════╗\n');
fprintf('║    FIXED vs TRACKER — WITH CELL TEMPERATURE MODEL            ║\n');
...
ETA_PANEL  = ETA_STC; % kept for night/fixed calc fallback
...
gamma_P = -0.004; % power temperature coefficient [1/°C]  (typical Si)
...
SupervisorParams.tracking_deadband   = 6.0;
...
%% ── LOAD IRRADIANCE DATA (_Data.csv) ─────────────────────────────────────
csv_filename = sprintf('%s_Data.csv', upper(loc_name));
csv_filepath = fullfile(pwd, csv_filename);
if ~isfile(csv_filepath)
    error('Irradiance file not found: %s', csv_filepath);
end
fprintf('Loading irradiance data: %s\n', csv_filename);
solar_data = readtable(csv_filepath);
fprintf('  ✓ %d records\n', height(solar_data));

%% ── LOAD TEMPERATURE & WIND DATA (MonthlyAvg CSV) ───────────────────────
avg_csv = sprintf('%s_MonthlyAvg.csv', upper(loc_name));
if ~isfile(avg_csv)
    % Try YTU_MonthlyAvg as fallback for Istanbul
    avg_csv = 'YTU_MonthlyAvg.csv';
end
if isfile(avg_csv)
    fprintf('Loading temperature/wind averages: %s\n', avg_csv);
    T_avg = readtable(avg_csv, 'VariableNamingRule', 'preserve');
    fprintf('  ✓ %d rows  (T2m and WS10m available: %s)\n\n', height(T_avg), ...
        string(ismember('WS10m', T_avg.Properties.VariableNames)));
else
    fprintf('  ℹ No MonthlyAvg CSV found — using default T_amb=20°C, WS=1.5 m/s\n\n');
    T_avg = [];
end

% Build lookup tables: T2m_lut(month, hour+1) and WS_lut(month, hour+1)
T2m_lut = ones(12, 24) * 20.0;  % default: 20°C
WS_lut  = ones(12, 24) * 1.5;   % default: 1.5 m/s
if ~isempty(T_avg)
    has_WS  = ismember('WS10m', T_avg.Properties.VariableNames);
    for m = 1:12
        rows_m = T_avg(T_avg.Month == m, :);
        [~, si] = sort(rows_m.Hour);
        rows_m  = rows_m(si, :);
        for hh = 0:23
            mask = (rows_m.Hour == hh);
            if any(mask)
                T2m_lut(m, hh+1) = rows_m.T2m(find(mask,1));
                if has_WS
                    WS_lut(m, hh+1) = max(0.5, rows_m.WS10m(find(mask,1)));
                end
            end
        end
    end
...
for d = 1:nDays
    ...
    csv_detail_file = fullfile(res_dir, ...
        sprintf('DetailedTimestep_%s_Month%02d_Temp.csv', loc_name, d));
    T_detail = table( ...
        time_vec, ...
...
fprintf('════════════════════════════════════════════════════════════════\n');
fprintf('  ANNUAL YIELD SUMMARY (WITH TEMPERATURE MODEL) — %s\n', Geo.Name);
fprintf('════════════════════════════════════════════════════════════════\n');
...
%% ── SAVE RESULTS .mat ────────────────────────────────────────────────────
mat_file = fullfile(res_dir, sprintf('%s_Results_Temp.mat', upper(loc_name)));
...
title(axT1, sprintf('Monthly Average Cell Temperature — %s, TR  (Faiman model)', Geo.Name), ...
    'FontName',FN,'FontSize',FSt,'FontWeight','bold');
legend(axT1,'Location','northwest','FontName',FN,'FontSize',FSr,'Box','on');
exportgraphics(figT1, fullfile(res_dir,'FigT1_CellTemperature.png'), 'Resolution',300);
close(figT1);
fprintf('  ✓ FigT1_CellTemperature.png\n');
...
title(axT2, sprintf('Monthly Temperature-Corrected Panel Efficiency — %s, TR', Geo.Name), ...
    'FontName',FN,'FontSize',FSt,'FontWeight','bold');
legend(axT2,'Location','best','FontName',FN,'FontSize',FSr,'Box','on');
exportgraphics(figT2, fullfile(res_dir,'FigT2_PanelEfficiency.png'), 'Resolution',300);
close(figT2);
fprintf('  ✓ FigT2_PanelEfficiency.png\n');

%% FIG T3 — Net Gain comparison: original (no T model) vs this run ────────
%  This figure shows that temp correction lowers power somewhat but
%  the GAIN ratio is robust.
...
title(axT3, sprintf('Monthly Net Gain with Cell Temperature Correction — %s, TR', Geo.Name), ...
    'FontName',FN,'FontSize',FSt,'FontWeight','bold');
exportgraphics(figT3, fullfile(res_dir,'FigT3_NetGain_withTemp.png'), 'Resolution',300);
close(figT3);
fprintf('  ✓ FigT3_NetGain_withTemp.png\n');

fprintf('\n╔══════════════════════════════════════════════════════════════╗\n');
fprintf('║  CompareYield_TMY_Temp COMPLETE                               ║\n');
fprintf('║  Results in: %s\n', res_dir);
fprintf('║  ✓ DetailedTimestep_*_Temp.csv   (per-second + temperature)   ║\n');
fprintf('║  ✓ %s_Results_Temp.mat                                        ║\n', upper(loc_name));
fprintf('║  ✓ FigT1_CellTemperature.png                                  ║\n');
fprintf('║  ✓ FigT2_PanelEfficiency.png                                  ║\n');
fprintf('║  ✓ FigT3_NetGain_withTemp.png                                 ║\n');
fprintf('╚══════════════════════════════════════════════════════════════╝\n');
